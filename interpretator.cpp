				...
				if ((tmp == ">") || (tmp == "<") || (tmp == "&&") || (tmp == "||") || (tmp == "=="))
				{
					...
					switch (tmp1)
					{
						case '>':
							...
							for (int i = 0; i < keywords.size(); i++)
							{
								if (tmp_next == keywords[i])
								{
									tmp_next = var_info[keywords[i]].value;
								}
								else
								{
									var_info[name].value = tmp_next;
								}
							}
							num1 = stoi(token);
							num2 = stoi(tmp_next);
...
void do_decleration_for_if(std::ifstream &fin, int type_index)
{
	...
	do {
		...
		if (token != "(")
		{
			std::cout << "your loop writen wrong" << std::endl;
		}
		else
		{
...
void do_decleration_for_while(std::ifstream &fin, int type_index)
{
	...
	do {
		...
		if (token != "(")
		{
			std::cout << "your if writen wrong" << std::endl;
		}
		else
		{
			fin >> token;	// popoxakani vra e
...
void do_interpretation(std::ifstream &fin)
{
	...
	while (!fin.eof())
	{
		...
		switch (case_ind)
		{
			...
			case 0:
				do_decleration_for_num(fin, case_ind);
				break;
			case 1:
				do_decleration_for_fnum(fin, case_ind);
				break;
			case 2:
